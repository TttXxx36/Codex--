...
- **🛠️ 实际完成的步骤 (Actual Steps)**：
  - `crates/codex-plus-data/src/storage.rs`：将 threads、archived 与 automation_runs 索引改为基于实际列的 Schema 探测；支持 `updated_at_ms`、`updated_at`、`created_at_ms` 及无时间列退化路径；对已有索引校验覆盖列和表达式，不匹配时按需重建；新增 `list_local_sessions_keyset`，以数据库内的 `(time, id)` 边界生成定向 WHERE/ORDER BY。
  - `crates/codex-plus-data/tests/storage_adapter.rs`：增加旧 threads Schema（仅 `updated_at`、仅 `created_at_ms`、无时间列）、无时间 automation_runs 索引以及连续 Keyset 页面无重复/无遗漏测试。
  - `apps/codex-plus-manager/src-tauri/src/commands.rs`：加入游标请求/响应字段、Base64URL 游标解码与大小/ID 校验；有 cursor 时按库各取 `limit + 1` 后归并排序，并对跨库重复 ID 做规范时间校验以过滤已在上一页出现的旧副本；保留无 cursor 的 offset 回退；Rust 编码使用与前端既有 `u/i` 载荷一致的格式。
